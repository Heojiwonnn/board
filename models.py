...
class DBManager:
    ...
    def get_members_by_page(self, page, members_per_page):
        try:
            self.connect()
            offset = (page - 1) * members_per_page
            sql = "SELECT * FROM member ORDER BY memberIdx DESC LIMIT %s OFFSET %s"
            self.cursor.execute(sql, (members_per_page, offset))
            return self.cursor.fetchall()  # 페이지에 해당하는 게시글 반환
        except mysql.connector.Error as error:
            print(f"페이지별 게시글 조회 실패: {error}")
            return []
    ...
    def get_member_by_memberIdx(self, memberIdx):
        try:
            self.connect()
            sql = "SELECT * FROM member WHERE memberIdx = %s"
            value = (memberIdx,) # 튜플 1개 일때
            self.cursor.execute(sql, value)
            return self.cursor.fetchone()
        except mysql.connector.Error as error:
            print(f"내용 조회 실패: {error}")
    ...
    def update_member(self,memberIdx, userid, username, password,memberLevel):
        try:
            self.connect()
            sql = """UPDATE member 
                    SET userid = %s, username = %s, password = %s, memberLevel = %s 
                    WHERE memberIdx = %s
                    """
            values = (userid, username, password, memberLevel, memberIdx)
            self.cursor.execute(sql, values)
            self.connection.commit()
            return True
        except mysql.connector.Error as error:
            self.connection.rollback()
            print(f"회원정보 수정 실패: {error}")
            return False
        finally:
            self.disconnect()
