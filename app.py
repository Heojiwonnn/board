...
def login():
    if request.method == 'POST':
        ...
        if success:
            ...
            session['memberLevel'] = result['memberLevel']
...
def logout():
    ...
    flash("로그아웃되었습니다.")  # flash 메시지 전달
    return redirect(url_for('index'))  # 'index' 페이지로 리다이렉트
...
def register():
    if request.method == 'POST':
        ...
        try:
            manager.connect()
            sql = "INSERT INTO member (userid, password, username, memberLevel) VALUES (%s, %s, %s, 0)"
...
def admin():
    # 요청된 페이지 번호 가져오기 (기본값: 1)
    page = request.args.get('page', 1, type=int)
    members_per_page = 10  # 페이지당 게시글 수

    # 게시글 총 개수와 현재 페이지의 게시글 가져오기
    total_members = manager.get_members_count()  # 총 게시글 수
    members = manager.get_members_by_page(page, members_per_page)

    # 총 페이지 수 계산
    total_pages = (total_members + members_per_page - 1) // members_per_page

    return render_template('admin.html', members=members, page=page, total_pages=total_pages)   

# 회원정보수정 
@app.route('/member/edit/<int:memberIdx>', methods=['GET', 'POST'])
def edit_member(memberIdx):
    member = manager.get_member_by_memberIdx(memberIdx)
    if request.method == 'POST':
        userid = request.form['userid']
        username = request.form['username']
        password = request.form['password']
        ...
        if manager.update_member(memberIdx, userid, username, password,memberLevel):
            return redirect("/admin")
        return "회원 정보 수정 실패", 400        
